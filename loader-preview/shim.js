 ...
 *
 * (c) 2026 Umami Audio. All rights reserved. No license to copy or re-implement.
...
(function () {
  ...
  var NATIVE_NAMES = [
    "loadConfig", "setVolume", "setBypass", "getState", "login", "cancelAuth", "logout",
    "userActivity", "openUrl", "selectLicense", "refreshLicenses", "getLicenses",
    "setGainLimit", "setAutoBypassRendering", "setShowExtraChannels",
    "setMeterFullscreen",
  ...
  /* Headphone profile: plain stereo L/R in and out. */
  function applyMeterLayoutHeadphone(name) {
    seed.meterInputNames = ["L", "R"];
    seed.meterOutputNames = ["L", "R"];
    seed.meterCfgInputCount = 2;
    seed.meterCfgOutputCount = 2;
    seed.meterInputHeightHints = [0, 0];
    seed.meterInputIsLfe = [false, false];
    seed.routingText =
      "[Demo · Headphone profile]\n" +
      name + " — stereo L/R through the headphone profile.";
    seed.routingTitle = "Active Routes (headphones)";
    seed.meterInputNoSignal = [];
  }
  ...
  var seed = {
    ...
    /* Demo license: a valid perpetual license so the licensing UI shows a realistic
       active state. All faked here — there is no real backend or license enforcement. */
    licenseStatus: "valid",
    licenseId: 1,
    ...
    /* Demo license list — faked so the license manager UI looks like the real product
       (one active perpetual license bound to this machine). No real entitlement data. */
    defaultLicenses: {
      licenses: [
        {
          id: 1,
  ...
  win.__JUCE__ = {
    backend: {
      ...
      emitEvent: function (name, ev) {
        ...
        try {
          if (pname === "userActivity" || pname === "uiPointerTrace") { done(null); return; }
          ...
          if (pname === "addDemoHeadphone") {
            /* Demo-only: the plugin's Add-headphone picker committed a model. Simulate
             * the download/load with the file-transfer banner, store the profile as a
             * local calibration so it appears in the dropdown, and load it. */
            var hpName = pm[0] != null ? String(pm[0]) : "";
            if (!hpName) { done(null); return; }
            var hpFile = hpName + ".soup-recipe";
            var hpPath = "C:\\Demo\\Headphones\\" + hpFile;
            var hpHave = false;
            for (var hi = 0; hi < seed.localCalsStored.length; hi++) {
              if (seed.localCalsStored[hi].filePath === hpPath) { hpHave = true; break; }
            }
            if (!hpHave) {
              seed.localCalsStored.push({
                fileName: hpFile, filePath: hpPath, calibrationName: hpName,
                jobId: "", createdAt: new Date().toISOString(),
                gated: false, gateReason: ""
              });
            }
            var emitHp = function (phase) {
              if (win.nativeBridge && win.nativeBridge.onCalibrationTransferProgress) {
                win.nativeBridge.onCalibrationTransferProgress({
                  filePath: hpPath, phase: phase, indeterminate: true
                });
              }
            };
            emitHp("load");
            setTimeout(function () {
              ...
              seed.configFileName = hpFile;
              seed.configFilePath = hpPath;
              applyMeterLayoutHeadphone(hpName);
              if (win.nativeBridge && win.nativeBridge.onLocalCalibrationList) {
                win.nativeBridge.onLocalCalibrationList({
                  calibrations: seed.localCalsStored.slice(),
                  currentConfig: hpFile,
                  currentConfigPath: hpPath,
                  currentConfigName: hpName
                });
              }
              emitHp("idle");
              pushState();
          ...
          if (pname === "loadLocalCalibration") {
            /* Re-selecting a demo local profile (only headphones exist locally here). */
            var lpPath = pm[0] != null ? String(pm[0]) : "";
            var lpItem = null;
            for (var li = 0; li < seed.localCalsStored.length; li++) {
              if (seed.localCalsStored[li].filePath === lpPath) { lpItem = seed.localCalsStored[li]; break; }
            }
            if (lpItem) {
              seed.configLoaded = true;
              seed.configFileName = lpItem.fileName;
              seed.configFilePath = lpItem.filePath;
              applyMeterLayoutHeadphone(lpItem.calibrationName);
              pushState();
            }
            done(null); return;
          }
  ...
  function injectOverrideStylesOnce() {
    ...
    style.textContent =
      ...
      /* 2. Calibration dropdown "+ Create new calibration..." entry only — the
       *    "+ Add headphone..." action row stays: its picker is part of the demo. */
      ".config-dropdown-item.action-item[data-action='create'] { display: none !important; }\n" +
      ...
      /* 5. Version / build message — hidden in the marketing demo (no real version to show). */
      "#build-info { display: none !important; }\n";
